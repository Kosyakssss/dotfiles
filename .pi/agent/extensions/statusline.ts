      ...
      (tui: TUI, editorTheme: EditorTheme, keybindings: KeybindingsManager) => {
        ...
        return new (class StatusEditor extends CustomEditor {
          ...
          override render(width: number): string[] {
            ...
            const fastStatus = getStatuses().get("fast-mode");
            const topRight = effort(
              ` ${PI_SYMBOL}${SEPARATOR}${model}${SEPARATOR}${thinking}${fastStatus ? `${SEPARATOR}${fastStatus}` : ""} `,
            );
            ...
            for (const [id, status] of statuses) {
              if (id === "prompt-stash" || id === "fast-mode") continue;
